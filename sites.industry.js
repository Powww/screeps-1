...
module.exports = {
	...
	runTerminal: function (rmColony) {
		if (Game.rooms[rmColony].terminal != null && Game.rooms[rmColony].terminal.my) {
			...
			// Add critical energy level to room's stockpile (to prevent sending to other rooms)
			let __Colony = require("util.colony");
			let energy_shortage_level = __Colony.getCriticalStockpile(_.get(Game, ["rooms", rmColony, "controller", "level"]));
			if (_.get(Memory, ["rooms", rmColony, "stockpile", "energy"]) < energy_shortage_level) {
				_.set(Memory, ["rooms", rmColony, "stockpile", "energy"],
					energy_shortage_level + _.get(Memory, ["rooms", rmColony, "stockpile", "energy"]));			
			}
			...
			if (room_energy_level < (energy_shortage_level * 1.1)) {
				...
				if (amount > 0) {
					...
					if (_.get(Memory, ["resources", "terminal_orders", `${rmColony}-critical_energy`]) == null)
						console.log(`<font color=\"#DC00FF\">[Terminals]</font> Creating critical energy order for ${rmColony} for ${amount} energy.`);
					_.set(Memory, ["resources", "terminal_orders", `${rmColony}-critical_energy`], 
						{ room: rmColony, resource: "energy", amount: amount, automated: true, priority: 1 });
					
				}
			} else {
				delete Memory["resources"]["terminal_orders"][`${rmColony}-critical_energy`];
			}
